...
(function(){
	...
	var socialIconsNumber = function(){
	    var out = 0;
	    jQuery('.mobile-social-icons .social-icons a').each(function(){
		out++;
	    });
	    return out;
	};
	...
	if (socialIconsNumber() < 1){
	    jQuery('.mobile-social-icons').css('min-height', '62px');
	}

	var windowViewport = jQuery(window).width(),
	    mobileTitleShown = jQuery('.mobile-title').length;
